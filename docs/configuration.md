...
All configuration is done in the `neon.yaml` file. This file is located in the `/home/neon/xdg/neon/neon.yaml` directory by default. After making changes to the file, you must restart the Neon core services for the changes to take effect. The easiest way to do so is through the Simple Docker Manager UI at `https://manager.neon-hub.local`.
...
Each skill has its own settings file located in the `/home/neon/xdg/neon/skills/$SKILL_NAME/settings.json` directory. These settings can be modified to change the behavior of the skill, either by editing the file directly or by using the Skill Configuration tool at `https://skill-config.neon-hub.local`. Some skills require you to restart the neon-skills service for the changes to take effect, although many skills automatically read the settings file each time and do not require a restart.
